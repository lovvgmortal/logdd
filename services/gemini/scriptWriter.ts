...
export const generateSectionScript = async (
  ...
  const context = globalPrompt || "Start of video";

  const systemPrompt = `${WRITER_SYSTEM_PROMPT}\nDNA CONTEXT: Tone: ${blueprint.analysis.linguistic_fingerprint.dominant_tones.join(', ')}`;
  const userPrompt = `${context}\n${getLanguageInstruction(language)}\nTASK: Write script for:\n${sectionDetails}\n${customInstruction}\nSOURCE: ${draft.script || draft.description}`;
